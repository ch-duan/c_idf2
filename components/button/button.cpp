...
void button_init(struct Button* handle, uint8_t (*pin_level)(uint16_t), uint8_t active_level, uint16_t button_id) {
  ...
  handle->multiple_click = 2;
  handle->long_ticks = LONG_TICKS;
}

/**
 * @brief: set the button long ticks
 * @param handle:  the button handle struct.
 * @param long_ticks  long_ticks
 * @return
 */
void button_set_long_ticks(struct Button* handle, uint16_t long_ticks) {
  if (handle != NULL) {
    handle->long_ticks = long_ticks / TICKS_INTERVAL;
  }
}
  ...
  switch (handle->state) {
    ...
    case 2:
      if (handle->button_level == handle->active_level) {  // press down again
        ...
        } else if (handle->repeat == handle->multiple_click) {
          handle->event = (uint8_t) MULTIPLE_CLICK;
          EVENT_CB(MULTIPLE_CLICK);  // repeat hit
        }
        handle->state = 0;
      }
      break;

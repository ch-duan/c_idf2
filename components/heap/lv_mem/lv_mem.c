 ...
 *   GLOBAL FUNCTIONS
 **********************/
static SemaphoreHandle_t memLock;
/**
 * Initialize the dyn_mem module (work memory and other variables)
 */
void lv_mem_init(uint8_t *addr) {
#if LV_MEM_CUSTOM == 0
  work_mem = (uint8_t *) addr;
  // #if LV_MEM_ADR == 0
...
void lv_mem_lock() {
  __disable_irq();
  xSemaphoreTake(memLock, portMAX_DELAY);
}

void lv_mem_unlock() {
  xSemaphoreGive(memLock);
  __enable_irq();
}
...
void *lv_mem_realloc(void *data_p, size_t new_size) {
  ...
  if (old_size == new_size) {
    // xSemaphoreGive(memLock);
    return data_p; /*Also avoid reallocating the same memory*/
  }

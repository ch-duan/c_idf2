...
uint32_t QMI8658_SLAVE_ADDRESS = QMI8658_H_SLAVE_ADDRESS;

HAL_StatusTypeDef I2C_Read(uint8_t reg, uint8_t *data, uint8_t size) {
  uint8_t cnt = 0;
  HAL_StatusTypeDef status = HAL_BUSY;
  do {
    status = HAL_I2C_Mem_Read(mI2C, QMI8658_SLAVE_ADDRESS << 1 | 0x01, reg, 1, data, size, 500);
    if (status != HAL_OK) {
      if (++cnt > 3) {
        break;
      }
...
int I2C_Write(uint8_t reg, uint8_t data) {
  ...
  HAL_StatusTypeDef status = HAL_BUSY;
  do {
    status = HAL_I2C_Mem_Write(mI2C, QMI8658_SLAVE_ADDRESS << 1 | 0x00, reg, 1, &data, 1, 500);
    if (status != HAL_OK) {
      if (++cnt > 3) {
        break;
      }
...
void QMI8658_config_acc(enum QMI8658_AccRange range, enum QMI8658_AccOdr odr, enum QMI8658_LpfConfig lpfEnable, enum QMI8658_StConfig stEnable) {
  ...
  I2C_Write(QMI8658_REG_CTRL2, ctl_dada);
  // set LPF & HPF
  I2C_Read(QMI8658_REG_CTRL5, &ctl_dada, 1);
  ctl_dada &= 0xf0;
  ...
  ctl_dada = 0x00;
  I2C_Write(QMI8658_REG_CTRL5, ctl_dada);
  // set LPF & HPF
}
...
void QMI8658_config_gyro(enum QMI8658_GyrRange range, enum QMI8658_GyrOdr odr, enum QMI8658_LpfConfig lpfEnable, enum QMI8658_StConfig stEnable) {
  ...
  if (stEnable == QMI8658St_Enable)
    ...
    ctl_dada = (unsigned char) range | (unsigned char) odr;
  I2C_Write(QMI8658_REG_CTRL3, ctl_dada);
...
void QMI8658_config_mag(enum QMI8658_MagDev device, enum QMI8658_MagOdr odr) {
  I2C_Write(QMI8658_REG_CTRL4, device | odr);
}
...
void QMI8658_config_ae(enum QMI8658_AeOdr odr) {
  ...
  I2C_Write(QMI8658_REG_CTRL6, odr);
}

unsigned char QMI8658_readStatus0(void) {
  unsigned char status[2];

  I2C_Read(QMI8658_REG_STATUS0, status, sizeof(status));
  // printf("status[0x%x	0x%x]\n",status[0],status[1]);

  return status[0];
}
/*!
 * \brief Blocking read of data status register 1 (::QMI8658_REG_Status1).
 * \returns Status byte \see STATUS1 for flag definitions.
 */
unsigned char QMI8658_readStatus1(void) {
  unsigned char status;

  I2C_Read(QMI8658_REG_STATUS1, &status, sizeof(status));
...
float QMI8658_readTemp(void) {
  ...
  I2C_Read(QMI8658_REG_TEMPEARTURE_L, buf, 2);
...
void QMI8658_read_acc_xyz(float acc_xyz[3]) {
  ...
  I2C_Read(QMI8658_REG_AX_L, buf_reg, 6);  // 0x19, 25
...
void QMI8658_read_gyro_xyz(float gyro_xyz[3]) {
  ...
  I2C_Read(QMI8658_REG_GX_L, buf_reg, 6);  // 0x1f, 31
...
int QMI8658_read_xyz(float acc[3], float gyro[3], unsigned int *tim_count) {
  ...
  if (tim_count) {
    ...
    I2C_Read(QMI8658_REG_TIMESTAMP_L, buf, 3);  // 0x18	24
  ...
  int ret = I2C_Read(QMI8658_REG_AX_L, buf_reg, 12);  // 0x19, 25
  if (ret != 0) {
    return -1;
  }
  raw_acc_xyz[0] = (short) ((unsigned short) (buf_reg[1] << 8) | (buf_reg[0]));
...
void QMI8658_read_xyz_raw(short raw_acc_xyz[3], short raw_gyro_xyz[3], unsigned int *tim_count) {
  ...
  I2C_Read(QMI8658_REG_AX_L, buf_reg, 12);  // 0x19, 25
...
void QMI8658_read_ae(float quat[4], float velocity[3]) {
  unsigned char buf_reg[14] = {0};
  short raw_q_xyz[4] = {0};
  short raw_v_xyz[3] = {0};

  I2C_Read(QMI8658_REG_Q1_L, buf_reg, 14);
...
void QMI8658_enableWakeOnMotion(void) {
  ...
  womCmd[0] = QMI8658_REG_CAL1_L;  // WoM Threshold: absolute value in mg (with 1mg/LSB resolution)
  womCmd[1] = threshold;
  womCmd[2] = (unsigned char) interrupt | (unsigned char) initialState | (blankingTime & blankingTimeMask);
  I2C_Write(QMI8658_REG_CAL1_L, womCmd[1]);
  I2C_Write(QMI8658_REG_CAL1_H, womCmd[2]);

  // QMI8658_doCTRL9Command(CTRL9_ConfigureWakeOnMotion);
  QMI8658_enableSensors(QMI8658_CTRL7_ACC_ENABLE);
  // while(1)
  //{
  //	I2C_Read(QMI8658_REG_Status1,&womCmd[0],1);
  //	if(womCmd[0]&0x01)
  //		break;
  // }
}
...
void QMI8658_disableWakeOnMotion(void) {
  QMI8658_enableSensors(QMI8658_CTRL7_DISABLE_ALL);
  I2C_Write(QMI8658_REG_CAL1_L, 0);
  // QMI8658_doCTRL9Command(CTRL9_ConfigureWakeOnMotion);
}
...
void QMI8658_enableSensors(unsigned char enableFlags) {
  ...
  I2C_Write(QMI8658_REG_CTRL7, enableFlags & QMI8658_CTRL7_ENABLE_MASK);
}
...
void checkout_addr() {
  ...
  QMI8658_SLAVE_ADDRESS = QMI8658_H_SLAVE_ADDRESS;
  I2C_Read(QMI8658_REG_WHOAMI, &qmi8658_chip_id, 1);
  printf("id is %d\r\n", qmi8658_chip_id);
  if (qmi8658_chip_id != 0x05) {
    QMI8658_SLAVE_ADDRESS = QMI8658_L_SLAVE_ADDRESS;
    I2C_Read(QMI8658_REG_WHOAMI, &qmi8658_chip_id, 1);
    printf("id is %d\r\n", qmi8658_chip_id);
  }
  printf("QMI I2C addr is %02X\r\n", QMI8658_SLAVE_ADDRESS);
}

unsigned char QMI8658_init(I2C_HandleTypeDef *hi2c) {
  mI2C = hi2c;
  unsigned char qmi8658_chip_id = 0x00;
  ...
  I2C_Read(QMI8658_REG_REVISION, &qmi8658_revision_id, 1);
  printf("QMI8658_REG_REVISION is %d\r\n", qmi8658_revision_id);

  if (qmi8658_chip_id == 0x05) {
    printf("Qmi8658_init slave=0x%x  QMI8658_REG_WHOAMI=0x%x 0x%x\n", QMI8658_SLAVE_ADDRESS, qmi8658_chip_id, qmi8658_revision_id);
    I2C_Write(QMI8658_REG_CTRL1, 0x60);
    ...
    I2C_Write(QMI8658_REG_CTRL5, 0x55);

    // unsigned char read_data = 0x00;
    // I2C_Read(QMI8658_REG_CTRL1, &read_data, 1);
    // printf("QMI8658_REG_CTRL1=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL2, &read_data, 1);
    // printf("QMI8658_REG_CTRL2=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL3, &read_data, 1);
    // printf("QMI8658_REG_CTRL3=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL4, &read_data, 1);
    // printf("QMI8658_REG_CTRL4=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL5, &read_data, 1);
    // printf("QMI8658_REG_CTRL5=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL6, &read_data, 1);
    // printf("QMI8658_REG_CTRL6=0x%x \r\n", read_data);
    // I2C_Read(QMI8658_REG_CTRL7, &read_data, 1);
    // printf("QMI8658_REG_CTRL7=0x%x \r\n", read_data);
...
void QMI8658_soft_reset(void) {
  I2C_Write(QMI8658_REG_RESET, 0xb0);
}

void QMI8658_read(uint8_t reg, uint8_t *data, uint8_t size) {
  I2C_Read(reg, data, size);
}
